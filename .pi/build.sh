...
# docker buildx build --tag beastheart --file .pi/Dockerfile --platform linux/arm64 --cache-from beastheart:cache --cache-to beastheart:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar beastheart
...
sudo bash -c "echo beastheart > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/beastheart.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/beastheart.img
